...
DEBOOTSTRAP_PARAMS="--no-check-gpg $DEBOOTSTRAP_PARAMS"
...
# --------------- KernJC - begin ---------------
echo "kernjc" | sudo tee $DIR/etc/hostname
# add user user
sudo chroot $DIR /bin/bash -c "useradd -m user -s /bin/bash"
sudo sed -i '/^user/ { s/:x:/::/ }' $DIR/etc/passwd
...
sudo umount /mnt/$DIR
# --------------- KernJC - begin ---------------
KERNJC_IMAGE_DIR=$(readlink -f ../workdir/images)
mkdir -p $KERNJC_IMAGE_DIR
sudo rm -rf $DIR
mv $RELEASE.img $KERNJC_IMAGE_DIR/rootfs.img
mv $RELEASE.id_rsa $KERNJC_IMAGE_DIR/id_rsa
mv $RELEASE.id_rsa.pub $KERNJC_IMAGE_DIR/id_rsa.pub
# create overlay image
qemu-img create -f qcow2 -F raw -b $KERNJC_IMAGE_DIR/rootfs.img $KERNJC_IMAGE_DIR/rootfs-base.qcow2
ssh-keygen -f "~/.ssh/known_hosts" -R "[127.0.0.1]:10021" 2>/dev/null
sudo ssh-keygen -f "/root/.ssh/known_hosts" -R "[127.0.0.1]:10021" 2>/dev/null
# reset the exit code
echo
# --------------- KernJC - end -----------------
